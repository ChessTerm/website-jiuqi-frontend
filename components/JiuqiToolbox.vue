  ...
  <div>
    <float-box icon="tools" title="久棋工具箱" width="16rem">
      <b-list-group class="mt-3 bg-transparent">
        <b-list-group-item v-b-modal:jtp-stateParser-modal button>
          State Parser
          <b-icon id="jtp-stateParser-help" icon="question-circle-fill" />
        </b-list-group-item>
        <b-list-group-item v-b-modal:jtp-nextStep-modal button>
          Next Step
          <b-icon id="jtb-nextStep-help" icon="question-circle-fill" />
        </b-list-group-item>
      </b-list-group>
    </float-box>
    <b-popover target="jtp-stateParser-help" triggers="hover" placement="right">
      <template v-slot:title>
        久棋工具箱：<strong>State Parser</strong>
      </template>
    ...
    <b-modal id="jtp-stateParser-modal" title="State Parser" @hidden="resetStateParser">
      ...
      <template v-slot:modal-footer="{ cancel }">
        <b-button variant="success" @click="stateParserExport">
          <span v-if="stateParserLoading"><b-spinner variant="white" small /> 加载中...</span>
          <span v-else><b-icon icon="box-arrow-up" /> 导出当前 <code>State</code></span>
        </b-button>
        <b-button variant="secondary" @click="cancel">
          <b-icon icon="x" /> {{ stateParserFinished ? '关闭' : '取消' }}
        </b-button>
        <b-button v-if="!stateParserFinished" variant="primary" :disabled="stateParserLoading"
                  @click="stateParserSubmit">
          <span v-if="stateParserLoading"><b-spinner variant="white" small /> 加载中...</span>
    ...
    <b-modal id="jtp-nextStep-modal" title="Next Step" @show="nextStepCheck">
      <transition name="fadeDown" leave-active-class="fadeOutUp" mode="out-in">
          ...
          <b-row>
            ...
            <b-col cols="12" sm="6">
              <b-form-group label="阶段" label-for="jtp-nextStep-input-stage">
                <b-form-select id="jtp-nextStep-input-stage" v-model="nextStepStage"
                               :options="nextStepStages" required :state="nextStepStageError" />
        ...
        <div v-else class="mb-2">
          <p class="lead text-success">
            <b-icon class="mr-2" icon="circle-fill" animation="throb"
                    font-scale="1.2" />
            程序正在运行中...
          </p>
      ...
      <transition name="fadeUp">
        <p v-if="nextStepError && !nextStepSuccessful" class="text-danger mb-0">
          错误：{{ nextStepError }}
        </p>
      </transition>
      <transition name="fadeUp">
        <p v-if="nextStepSuccessful" class="text-success mb-0">
          程序已完成运行！
        </p>
      </transition>
      <template v-slot:modal-footer="{ cancel }">
        ...
        <b-button v-b-toggle:jtp-nextStep-sidebar variant="info">
          <b-icon icon="square-half" /> SideBar
        </b-button>
        <b-button variant="secondary" @click="cancel">
          <b-icon icon="x" /> 关闭
        </b-button>
        <b-button v-if="!nextStepStarted" variant="primary"
                  :disabled="nextStepLoading" @click="nextStepStart">
          <span v-if="nextStepLoading"><b-spinner variant="white" small /> 加载中...</span>
          <span v-else><b-icon icon="check2" /> 提交</span>
        </b-button>
        <b-button v-else variant="danger" @click="nextStepStop">
          <span v-if="nextStepLoading"><b-spinner variant="white" small /> 加载中...</span>
          <span v-else><b-icon icon="exclamation-octagon" /> 结束进程</span>
        </b-button>
      </template>
    </b-modal>
    <b-sidebar id="jtp-nextStep-sidebar" title="Next Step" right
               shadow="lg" @shown="nextStepCheck">
...
export default Vue.extend({
  ...
  computed: {
    nextStepPlayerError(): boolean|null {
      if (this.nextStepError && this.nextStepError.includes("player")) return false
      return null
    },
    nextStepStageError(): boolean|null {
      if (this.nextStepError && this.nextStepError.includes("stage")) return false
      return null
    },
