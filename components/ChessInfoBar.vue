<template>
  <b-sidebar id="sidebar-info" shadow="lg">
    <template v-slot:title>
      <b-icon icon="info-circle" /> 棋盘信息
    </template>
    <div class="px-3 py-2">
      <div v-if="board.id" class="text-left">
        ...
        <p v-if="user.id !== board.user.id">
          <strong>棋盘码：</strong>{{ board.user.id }}
        </p>
        ...
        <div v-if="user.id">
          ...
          <p v-if="user.admin === true" class="text-primary strong">
            管理员账号
          </p>
        </div>
      </div>
      <p v-else class="mt-5">
        <b-spinner variant="primary" small /> 棋盘信息加载中...
      </p>
    </div>
  </b-sidebar>
</template>
